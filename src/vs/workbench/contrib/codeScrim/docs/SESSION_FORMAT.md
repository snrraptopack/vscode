...
Browser replay is an optional passive track of timestamped DOM states plus visibility records. Visibility records identify which instructor browser page occupied the teaching surface. Each state is a full serialized document captured through the preloaded isolated-world helper with scripts and event-handler attributes stripped and live form state persisted. Mutation, input, change, scroll, and navigation signals create settled states between the initial and final checkpoints, so fetch-driven rendering and instructor interaction advance on the lesson clock. States are deduplicated by document, URL, title, and scroll position and content-addressed into the package blob store. Replay reconciles successive states into one persistent sandboxed document rather than reloading the iframe. Replaying or seeking never navigates to the recorded URL or evaluates recorded page scripts.
