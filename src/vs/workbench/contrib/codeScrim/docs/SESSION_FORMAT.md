...
Browser replay is an optional passive track made of semantic state events plus sparse thumbnails. Visibility records identify which instructor browser page occupied the teaching surface. Events reconstruct the recorded presentation state — navigation, title, zoom, device viewport, and scroll offset — in a live read-only Integrated Browser at replay time; they never carry executable page content. Thumbnails are JPEG previews captured at navigation and recording boundaries for timeline scrubber display only. Replaying or seeking never evaluates recorded page content.
