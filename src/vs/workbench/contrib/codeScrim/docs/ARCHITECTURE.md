...
The current workspace/editor slice lazily materializes checkpoint files and timeline-created files as `codescrim-replay:` resources backed by replay-owned Monaco text models. Recorded filesystem changes, text changes, active resources, and selections are applied to that isolated tree on the recording's monotonic clock. The native learner surface binds directly to those models and exposes a themed virtual tree, multi-file tabs, manual inspection, and arbitrary seek. All replay mutations are serialized so a timer tick, scrub, restart, or manual file switch cannot dispose a model another operation is still using. Authoritative post-edit text anchors prevent incremental range drift from corrupting later frames.

Passive instructor models retain their recorded language for native tokenization but are not synchronized to extension-host language services as editable workspace documents. Therefore live lint/type diagnostics are deliberately absent from this slice. Deterministic instructor squiggles will come from a timestamped captured-diagnostics track; optional live learner diagnostics will belong to the future sandbox overlay. Save events remain timeline markers during passive playback, so replay never writes instructor state into the creator's workspace or depends on files that remain on the creator's disk. Package loading, indexed checkpoints, captured diagnostics, and learner overlays remain future layers around this native-model core.
