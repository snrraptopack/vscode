...
- Popup and target-blank pages whose opener is a CodeScrim page stay inside the standalone author browser as native browser tabs. Open, close, activation, committed URL/title, and editor/browser focus transitions have dedicated timeline records; DOM snapshots no longer have to impersonate tab or surface state. Committed background pages are captured as well, so an external target-blank navigation is ready before the instructor activates it.
- Browser replay is DOM-state reconstruction, not video and not live re-navigation: a dedicated learner auxiliary window presents the recorded tab set, parses the active page state as trusted passive HTML, and reconciles it into one persistent sandboxed iframe. The frame document is not replaced between states, avoiding white flashes and preserving the continuity expected from editor replay. Recorded page scripts are never executed. Readable external stylesheets are embedded at capture time; opaque cross-origin assets require the bounded resource/visual fallback track rather than a learner-time network request.
